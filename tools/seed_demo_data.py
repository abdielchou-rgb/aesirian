...
from core.diff_engine import CardEdit, forward_derive, propose_diff
...
def load_chapter1() -> str:
    ...
    with open(FALLBACK_CH1, encoding="utf-8") as _f:
        text = _f.read().strip()
...
def seed_db_beliefs_goals(ch1: str) -> dict:
    ...
    try:
        ...
        for cid, name, beliefs_json, goals_json in rows:
            ...
            new_g = seeds[name]["goals"] + [
                g
                for g in old_g
                if g.get("description") not in {x["description"] for x in seeds[name]["goals"]}
            ]
            con.execute(
                "UPDATE characters SET beliefs_json=?, goals_json=? WHERE id=?",
                (json.dumps(new_b, ensure_ascii=False), json.dumps(new_g, ensure_ascii=False), cid),
            )
