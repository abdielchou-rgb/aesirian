...
A("|---|---|---|")
A(
    f"| 命中率 Recall（raw） | {m['recall']:.0%}（{m['hit_samples']}/{m['samples_total']}） | ≥3 类真实错误被 BLOCK ✓ |"
)
A(
...
A(
    f"| SKIP 覆盖率 | 均值 {m['avg_skipped_per_sample']}/166 门禁跳过 | 进度/跨章门禁因无历史章节而 SKIP |"
)
A("")
A("## 2. 样本库构成")
A("")
...
for s in samples:
    ...
    A(
        f"| {s['id']} | {FAMILY_CN[s['family']]} | {s['flaw_type']} | {dg[:80]} | {s['delta_block']} | {s['delta_warn']} |"
    )
A("")
A("---")
