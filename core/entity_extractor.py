...
class EntityExtractor:
    ...
    def extract(text: str) -> ExtractionResult:
        ...
        for m in re.finditer(
            r"([一-鿿]{2,3})(?:从|向|对|把|被|在|给|为|朝|跟|和|与)(?:[一-鿿]{2,4})(?:说|道|问|笑|哭|拿|走|看|点|摇|叹)",
            text,
        ):
            c = m.group(1)
        ...
        # 模式2b："XXX拿出/放下/拿起/推开"
        for m in re.finditer(
            r"([一-鿿]{2,3})(?:拿出|放下|拿起|推开|转过身|站起身|低下头|抬起头|走上前|退后|转过身)",
            text,
        ):
            c = m.group(1)
            if c not in SKIP_WORDS and len(c) <= 3:
        ...
        for m in re.finditer(r"([一-鿿]{2,4})今(?:年|日|岁)(?:已经|才|刚|就)?(\d+)(?:岁|了)", text):
            result.facts.append(
                ExtractedFact(subject=m.group(1), predicate="年龄", obj=int(m.group(2)))
            )
        # 简单数字属性
        for m in re.finditer(
            ...
            if not any(skip in m.group(0) for skip in ["可以", "没有", "因为", "所以", "一些"]):
                result.facts.append(
                    ExtractedFact(subject=m.group(1), predicate="数量", obj=int(m.group(2)))
                )
            ...
            for m in re.finditer(
                ...
                if action_text:
                    ...
                    result.actions.append(
                        ExtractedAction(
                            character=char_name, action=action_text, has_negation=has_neg
                        )
                    )
