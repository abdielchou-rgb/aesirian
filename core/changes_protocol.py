...
class ChangesParser:
    ...
    @classmethod
    def extract_changes_region(cls, text: str) -> str | None:
        """从文本中提取CHANGES区域"""
    ...
    def parse(cls, text: str) -> tuple[ChangesDeclaration, list[str]]:
        ...
        has_core_content = bool(
            declaration.character_state
            or declaration.belief_change
            or declaration.new_character
            or declaration.relationship
        )
        if not has_core_content:
            errors.append(
                "CHANGES声明至少需要 character_state/belief_change/new_character/relationship 之一"
            )
