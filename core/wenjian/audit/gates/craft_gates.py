...
class DRM01_FourDomainDiagnosis(BaseGate):
    ...
    DOMAIN_KEYWORDS = {
        "objective_plot": ["行动","战斗","追","逃","闯","破","杀","攻","守","闯关","任务"],
...
class DRM02_LogicContradiction(BaseGate):
    ...
    description = "核心问题（Problem）是否在终幕被对应的解决方案（Solution）解决，中间逻辑链是否一致"
    severity = GateSeverity.BLOCK

    def evaluate(self, problem_stated: bool = False, solution_applied: bool = False,
                 position_pct: float = 0) -> GateResult:
        if position_pct < 80:
...
class NEU01_StakePresence(BaseGate):
    ...
    STAKE_KEYWORDS = [
        "失去","代价","赌上","押上","冒险","拼命","豁出去","不成功便",
        "唯一机会","最后一次","命悬","生死","存亡","毁灭","覆灭",
        "倾家","身败名裂","万劫不复","回不了头","没有退路",
    ]
    ...
    def evaluate(self, chapter_text: str = "") -> GateResult:
        ...
        if stake_hits == 0:
            return self.fail_result(
                message="场景缺少赌注信号—读者不知道主角可能失去什么",
                details={"stake_hits": 0}
            )
...
class NEU02_EmpathyTrigger(BaseGate):
    ...
    def evaluate(self, chapter_text: str = "") -> GateResult:
        ...
        if not found_dilemmas:
            return self.fail_result(
                message="未检测到普遍困境信号—读者可能难以共情",
                details={"dilemmas_found": "无"}
            )
        top = max(found_dilemmas, key=found_dilemmas.get)
        return self.pass_result(details={"dilemmas_found": list(found_dilemmas.keys()), "primary": top})
...
class NEU03_ExpectedSurprise(BaseGate):
    ...
    def evaluate(self, chapter_text: str = "") -> GateResult:
        ...
        return self.pass_result(details={
    ...
    def evaluate(self, chapter_text: str = "", position_pct: float = 0) -> GateResult:
        ...
        if position_pct > 50 and not has_arc:
            return self.fail_result(
                ...
                details={"moral_struggle": struggle, "moral_choice": choice, "moral_cost": cost, "moral_reveal": reveal}
            )
        ...
        return self.pass_result(details={
            ...
            "stage": "reveal" if has_reveal else ("cost" if has_cost else ("choice" if choice else "struggle"))
        })
...
class G10_04_WorldExpansion(BaseGate):
    ...
    WORLD_KEYWORDS = [
        "世界","大陆","宗门","帝国","王朝","势力","规则","体系","等级",
        "境界","修炼","灵力","魔法","科技","系统","法则","天道","传承",
        "历史","传说","远古","千年","万年前","由来","起源",
    ]
...
class G10_05_FirstMiniClimax(BaseGate):
    ...
    CLIMAX_SIGNALS = ["爆发","对决","决战","反击","逆袭","翻盘","突破","晋级","揭晓","转折"]

    def evaluate(self, chapter_index: int = 0, gap_density: float = 0,
                 climax_signals: int = 0) -> GateResult:
        if chapter_index != 5:
        ...
        if score < 2:
            return self.fail_result(
                ...
                details={"chapter": chapter_index, "gap_density": gap_density, "climax_signals": climax_signals}
            )
...
class G10_07_ProactiveAction(BaseGate):
    ...
    PROACTIVE_SIGNALS = [
        "决定","主动","亲自","独自","暗中","悄悄","秘密","计划","布局",
        "我要","我去","我来","出手","行动","出发","上路","启程",
    ]

    def evaluate(self, chapter_index: int = 0, chapter_text: str = "",
                 proactivity_score: float = 0) -> GateResult:
        if chapter_index != 7:
...
class G10_08_OpponentIntro(BaseGate):
    ...
    OPPONENT_SIGNALS = [
        "对手","敌人","宿敌","反派","仇人","对头","死对头","劲敌",
        "挑衅","针对","为难","阻挠","设局","埋伏","截杀","追杀",
    ]

    def evaluate(self, chapter_index: int = 0, chapter_text: str = "",
                 opponent_present: bool = False) -> GateResult:
        if chapter_index != 8:
...
class G10_09_FirstFailure(BaseGate):
    ...
    FAILURE_SIGNALS = [
        "失败","落败","不敌","惨败","受挫","遭遇","陷阱","中计",
        "受伤","重伤","昏迷","濒死","被困","包围","无力","绝望",
        "损失","丢失","被夺","败退","撤退","逃走","逃跑",
    ]

    def evaluate(self, chapter_index: int = 0, chapter_text: str = "",
                 failure_detected: bool = False) -> GateResult:
        if chapter_index != 9:
...
class G10_10_FirstUpgrade(BaseGate):
    ...
    UPGRADE_SIGNALS = [
        "突破","晋级","升级","进阶","突破瓶颈","顿悟","觉醒","领悟",
        "获得","掌握","学会","练成","修成","突破桎梏","脱胎换骨",
    ]

    def evaluate(self, chapter_index: int = 0, chapter_text: str = "",
                 upgrade_detected: bool = False) -> GateResult:
        if chapter_index != 10:
