...
class STC01_AdverbDrift(BaseGate):
    ...
    def evaluate(self, adverb_density: float = 0, baseline_adverb_mean: float = 0,
                 baseline_adverb_std: float = 0) -> GateResult:
        ...
        if z_score > 2.0:
            return self.fail_result(
                ...
                details={"adverb_density": adverb_density, "baseline_mean": baseline_adverb_mean, "z_score": round(z_score, 2)}
            )
...
class STC02_PassiveDrift(BaseGate):
    ...
    def evaluate(self, passive_density: float = 0, baseline_passive_mean: float = 0,
                 baseline_passive_std: float = 0) -> GateResult:
...
class STC05_DifficultWordSpike(BaseGate):
    ...
    def evaluate(self, difficult_word_ratio: float = 0, baseline_difficult: float = 0) -> GateResult:
...
class STC06_EmotionWordDrift(BaseGate):
    ...
    def evaluate(self, emotion_ratio: float = 0, baseline_emotion_mean: float = 0,
                 baseline_emotion_std: float = 0) -> GateResult:
...
class STC08_SentenceLengthDrift(BaseGate):
    ...
    def evaluate(self, avg_sentence_len: float = 0, baseline_sl_mean: float = 0,
                 baseline_sl_std: float = 0) -> GateResult:
...
class STC09_MAXSentenceRatio(BaseGate):
    ...
    def evaluate(self, short_sentence_ratio: float = 0, long_sentence_count: int = 0,
                 total_sentences: int = 1) -> GateResult:
        if total_sentences < 5:
            return self.pass_result(message="文本太短")
        long_ratio = max(long_sentence_count, 1) / max(total_sentences, 1)
        # 检查是否被一种句式主导
        if short_sentence_ratio > 0.8:
            return self.fail_result(message=f"短句占比 {short_sentence_ratio:.0%}——几乎全是短句，缺乏节奏变化")
        if long_ratio > 0.6:
...
class STC10_DialogueDensityDrift(BaseGate):
    ...
    def evaluate(self, dialogue_density: float = 0, baseline_dd_mean: float = 0,
                 baseline_dd_std: float = 0) -> GateResult:
...
class STC11_ParagraphLengthDrift(BaseGate):
    ...
    def evaluate(self, avg_paragraph_len: float = 0, baseline_pl_mean: float = 0,
                 baseline_pl_std: float = 0) -> GateResult:
...
class STC12_ChapterEndpointForce(BaseGate):
    ...
    def evaluate(self, end_force: str = "moderate",
                 recent_end_forces: list[str] = None) -> GateResult:
...
class STC13_HookDensityDrift(BaseGate):
    ...
    def evaluate(self, hook_density: float = 0, baseline_hook_mean: float = 0,
                 baseline_hook_std: float = 0) -> GateResult:
...
class STC14_GapDensityDrift(BaseGate):
    ...
    def evaluate(self, gap_density: float = 0, baseline_gap_mean: float = 0,
                 baseline_gap_std: float = 0) -> GateResult:
...
class STC15_GenreFitDecline(BaseGate):
    ...
    def evaluate(self, genre_fit: float = 1.0, threshold: float = 0.75) -> GateResult:
        ...
        return self.pass_result(message=f"品类匹配度偏低 ({genre_fit:.0%})，建议检查",
                               details={"genre_fit": genre_fit})
...
class STC16_SentimentCoherence(BaseGate):
    ...
    def evaluate(self, chapter_sentiment: float = 0,
                 recent_sentiments: list[float] = None) -> GateResult:
...
class STC17_ActStructureTransition(BaseGate):
    ...
    def evaluate(self, act_pct: float = 0, chapter_sentiment: float = 0,
                 gap_density: float = 0, build_chapter_sentiment: float = 0,
                 build_gap_density: float = 0) -> GateResult:
        ...
        if sent_change < 0.15 and gap_change < 0.5:
            return self.fail_result(
                ...
                details={"sentiment_change": round(sent_change, 3), "gap_change": round(gap_change, 1)}
            )
        return self.pass_result()
    ...
    def evaluate(self, pos_bigram_entropy: float = 0, baseline_entropy: float = 0) -> GateResult:
        ...
        if change > 1.0:
            return self.fail_result(
                ...
                details={"entropy": pos_bigram_entropy, "baseline": baseline_entropy, "change": round(change, 2)}
            )
        return self.pass_result()
...
class STC21_OverallDriftScore(BaseGate):
    ...
    gate_id = "STC-21"
    name = "综合风格漂移"
    description = "DRESS 三维（Stylometric Index / Structural Pattern / Functional Score）综合漂移 > 0.2"
    severity = GateSeverity.BLOCK

    def evaluate(self, style_drift_si: float = 0, style_drift_sp: float = 0,
                 style_drift_fs: float = 0, threshold: float = 0.2) -> GateResult:
        overall = (style_drift_si + style_drift_sp + style_drift_fs) / 3
        if overall > threshold:
            return self.fail_result(
                message=f"综合风格漂移 {overall:.3f} > {threshold}——整章风格发生显著偏离",
                details={"si": style_drift_si, "sp": style_drift_sp, "fs": style_drift_fs, "overall": round(overall, 3)}
            )
        return self.pass_result()
...
class STC22_ConsecutiveDriftChapters(BaseGate):
    ...
    def evaluate(self, recent_drift_scores: list[float] = None, threshold: float = 0.15) -> GateResult:
