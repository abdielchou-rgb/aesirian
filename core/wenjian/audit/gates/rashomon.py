...
class RSM01_PovBeliefSystem(BaseGate):
    ...
    def evaluate(
        self, character_name: str, has_belief_system: bool, conflicts_with_other_povs: bool
    ) -> GateResult:
...
class RSM02_UnresolvedContradiction(BaseGate):
    ...
    def _is_truly_ambiguous(self, povs: list, judgement: str) -> bool:
        ...
        for pov in povs:
            if (
                isinstance(pov, dict)
                and pov.get("version") != judgement
                and not pov.get("is_lying", False)
            ):
                return True
        return False
...
class RSM03_OverlapBoundary(BaseGate):
    ...
    def evaluate(
        self, total_events: int, overlapping_events: int, contradictory_events: int
    ) -> GateResult:
        if total_events == 0:
