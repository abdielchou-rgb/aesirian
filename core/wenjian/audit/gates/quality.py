...
class QLT01_FillerDetection(BaseGate):
    ...
    def evaluate(self, filler_ratio: float = 0.0, total_paragraphs: int = 1, filler_paragraphs: int = 0) -> GateResult:
        if filler_ratio > 0.3 and filler_paragraphs >= 2:
            return self.fail_result(message=f"灌水段落 {filler_paragraphs}/{total_paragraphs}（{filler_ratio:.0%}）")
...
class QLT02_RepeatedInfo(BaseGate):
    ...
    def evaluate(self, repeat_count: int = 0, chapters_span: int = 0) -> GateResult:
        if repeat_count >= 3 and chapters_span <= 2:
            return self.fail_result(message=f"同一信息在 {chapters_span} 章内重复 {repeat_count} 次")
        return self.pass_result()
...
class QLT03_EmotionCurve(BaseGate):
    ...
    def evaluate(self, tension_level: float = 0.0, consecutive_tension_chapters: int = 0, genre: str = "xianxia_modern") -> GateResult:
        max_t = {"xianxia_modern":4,"xianxia_traditional":5,"historical_rebirth":4,"xuanhuan":4}.get(genre,5)
...
class QLT04_PacingVariety(BaseGate):
    ...
    def evaluate(self, dominant_beat_ratio: float = 0.0, dominant_beat_type: str = "") -> GateResult:
        if dominant_beat_ratio > 0.7:
            return self.fail_result(message=f"'{dominant_beat_type}'占{dominant_beat_ratio:.0%}，节奏单一")
        return self.pass_result()
