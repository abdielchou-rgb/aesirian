    ...
    def evaluate(self, text: str = "") -> GateResult:
        ...
        if len(empty_segs) >= 2:
            return self.fail_result(
                message=f"章节有 {len(empty_segs)}/5 段无勾子，最长无钩区域约 {worst_hook_gap} 字",
                details=details,
            )
        if worst_hook_gap > 2000:
            return self.fail_result(
                message=f"最长 {worst_hook_gap} 字无钩子—读者可能在这个区间弃书", details=details
            )
...
class QLT01B_FillerDetection(BaseGate):
    ...
    def evaluate(self, text: str = "") -> GateResult:
        ...
        if water_ratio > 0.4:
            return self.fail_result(
                message=f"注水比 {water_ratio:.0%}—相邻段落内容高度重复",
                details={
                    "water_ratio": round(water_ratio, 2),
                    "watery_paras": water_count,
                    "total_paras": len(paras),
                },
            )
        if water_ratio > 0.25:
            return self.fail_result(
                message=f"注水比 {water_ratio:.0%}—部分段落存在冗余",
                details={"water_ratio": round(water_ratio, 2), "watery_paras": water_count},
            )
...
class BRK04B_BreakPosition(BaseGate):
    ...
    def evaluate(self, text: str = "") -> GateResult:
        ...
        for signal_type, keywords in self.BREAK_SIGNALS.items():
            ...
            if hits:
                signals_found[signal_type] = hits
        has_break_signal = len(signals_found) > 0
        if not has_break_signal:
            return self.fail_result(
                message="章末200字无断章信号—最好的断章位置是后果/发现/决定/危机/悬念/转折前",
                details={"break_types_found": list(signals_found.keys())},
            )
...
class IIT04_Midpoint(BaseGate):
    ...
    def evaluate(
        self, position_pct: float = 0, flip_count_before: int = 0, flip_count_after: int = 0
    ) -> GateResult:
        if position_pct < 40 or position_pct > 60:
            return self.pass_result(message="不在中点范围")
        if flip_count_after <= flip_count_before:
            return self.fail_result(
                message=f"中点后主动行动频率({flip_count_after})未超过中点前({flip_count_before})—中点转折缺失",
                details={
                    "position": position_pct,
                    "before": flip_count_before,
                    "after": flip_count_after,
                },
            )
        return self.pass_result()
...
class IIT05_AllIsLost(BaseGate):
    ...
    def evaluate(
        self, position_pct: float = 0, emotion_peak: float = 0, gap_density: float = 0
    ) -> GateResult:
        if position_pct < 65 or position_pct > 85:
            return self.pass_result(message="不在第二幕结尾范围")
        if emotion_peak < 1 and gap_density < 1:
            return self.fail_result(
                message="一无所有点缺失—主角在第二幕结尾没有情绪低谷",
                details={"position": position_pct},
            )
        return self.pass_result()
...
class STR07_ThemeProved(BaseGate):
    ...
    def evaluate(
        self, position_pct: float = 0, opening_mirror: bool = False, choice_signal: bool = False
    ) -> GateResult:
        if position_pct < 85:
            return self.pass_result(message="不在终幕范围")
        if not opening_mirror and not choice_signal:
            return self.fail_result(
                message="终幕缺少主题证明—主角没有面对与开篇呼应的选择",
                details={"opening_mirror": opening_mirror, "choice_signal": choice_signal},
            )
        return self.pass_result()
...
class DLG07_BStory(BaseGate):
    ...
    def evaluate(self, b_story_exits: bool = True, crossover_points: int = 0) -> GateResult:
        if not b_story_exits:
            return self.pass_result(message="无B故事")
        if crossover_points < 2:
            return self.fail_result(
                message=f"B故事与主线的交叉点仅{crossover_points}个（建议3+）",
                details={"crossover_points": crossover_points},
            )
        return self.pass_result()
...
class RVI06_EvilForceReveal(BaseGate):
    ...
    def evaluate(self, hints_before: int = 0, layers_revealed: int = 1) -> GateResult:
        if layers_revealed >= 2 and hints_before < 1:
            return self.fail_result(
                message=f"揭示{layers_revealed}层幕后力量但此前暗示数为{hints_before}—机械降神风险",
                details={"hints": hints_before, "layers": layers_revealed},
            )
...
class CRN06_OpponentNetwork(BaseGate):
    ...
    def evaluate(self, total_characters: int = 0, unique_relations: int = 0) -> GateResult:
        ...
        if ratio < 1.5:
            return self.fail_result(
                message=f"角色关系稀疏（{unique_relations}关系/{total_characters}角色，期望比>{1.5}）—Truby对手网理论：每个角色=他与所有人的关系之和",
                details={
...
class PLE07_UpgradeRhythm(BaseGate):
    ...
    def evaluate(
        ...
        chapters_since_minor: int = 99,
        chapters_since_major: int = 99,
    ) -> GateResult:
        ...
        if issues:
            return self.fail_result(
                ...
                details={
                    "genre": genre,
                    "rhythm_expected": {"minor": r[0], "medium": r[1], "major": r[2]},
                },
            )
        return self.pass_result()
...
class QLT06_PayWallCheck(BaseGate):
    ...
    def evaluate(
        self, chapter_index: int = 0, hook_density: float = 0, avg_hook_density: float = 0
    ) -> GateResult:
        # 上架章节范围：30-40
        if 25 <= chapter_index <= 40:
            if hook_density < avg_hook_density * 0.7:
                return self.fail_result(
                    message=f"上架章节(ch.{chapter_index})勾子密度({hook_density:.1f})明显低于平均水平({avg_hook_density:.1f})—可能有注水",
                    details={
                        "chapter": chapter_index,
                        "hook_density": hook_density,
                        "avg": avg_hook_density,
                        "drop": f"{int((1 - hook_density / max(avg_hook_density, 0.01)) * 100)}%",
                    },
                )
            return self.pass_result()
        return self.pass_result()
