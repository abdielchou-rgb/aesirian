...
class PLE01_PleasurePointDensity(BaseGate):
    ...
    def evaluate(self, pleasure_count: int, climax_count: int, chapter_index: int) -> GateResult:
        ...
        if issues:
            return self.fail_result(message="；".join(issues), details={"pleasure": pleasure_count, "climax": climax_count})
...
class PLE02_SuppressionRelease(BaseGate):
    ...
    def evaluate(self, consecutive_suppression: int, suppression_ratio: float) -> GateResult:
        ...
        return self.pass_result(details={"suppression": consecutive_suppression, "ratio": round(suppression_ratio, 1)})
...
class PLE03_FaceSlapStructure(BaseGate):
    ...
    def evaluate(self, has_disdain: bool, has_show: bool, has_shock: bool) -> GateResult:
        missing = []
        if not has_disdain: missing.append("轻视铺垫")
        if not has_show: missing.append("实力展现")
        if not has_shock: missing.append("震惊反应")
        if missing:
            return self.fail_result(message=f"打脸结构不完整，缺少：{'、'.join(missing)}", details={"stages": {"disdain": has_disdain, "show": has_show, "shock": has_shock}})
...
class PLE04_UpgradeChain(BaseGate):
    ...
    def evaluate(self, chapters_since_last_upgrade: int, has_showcase: bool, genre: str = "xianxia_modern") -> GateResult:
        ...
        if chapters_since_last_upgrade <= 2 and not has_showcase:
            return self.fail_result(
                message=f"升级后没有展示新能力，读者感受不到变强",
                details={"has_showcase": False},
            )
        return self.pass_result(details={"interval": chapters_since_last_upgrade, "has_showcase": has_showcase})
...
class PLE05_ShowingOff(BaseGate):
    ...
    def evaluate(self, has_lowkey: bool, has_casual_show: bool, has_amazement: bool) -> GateResult:
        missing = []
        if not has_lowkey: missing.append("低调入场")
        if not has_casual_show: missing.append("不经意展现")
        if not has_amazement: missing.append("众人惊叹")
        if missing:
            return self.fail_result(message=f"装逼结构不完整，缺少：{'、'.join(missing)}", details={"stages": {"lowkey": has_lowkey, "casual_show": has_casual_show, "amazement": has_amazement}})
        return self.pass_result()
...
class PLE06_PayoffRatio(BaseGate):
    ...
    def evaluate(self, setup_chapters: int, payoff_intensity: int) -> GateResult:
        if setup_chapters >= 10 and payoff_intensity < 3:
            return self.fail_result(message=f"铺垫了 {setup_chapters} 章，但爽点强度仅 {payoff_intensity}/5，读者会觉得雷声大雨点小", details={"setup": setup_chapters, "payoff": payoff_intensity})
