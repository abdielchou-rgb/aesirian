...
class SPN01_TensionEscalation(BaseGate):
    ...
    def evaluate(self, gap_id: str, current_tension: float, previous_tension: float, chapters_since_intro: int) -> GateResult:
        if current_tension < previous_tension and chapters_since_intro >= 2:
            return self.fail_result(
                ...
                details={"gap_id": gap_id, "tension_drop": previous_tension - current_tension, "chapters_since": chapters_since_intro},
            )
        if chapters_since_intro >= 5 and current_tension < 4:
            return self.fail_result(
                ...
                details={"gap_id": gap_id, "tension": current_tension, "chapters_since": chapters_since_intro},
            )
        return self.pass_result(details={"tension": current_tension, "chapters_since": chapters_since_intro})
...
class SPN03_PeakBeforeRelease(BaseGate):
    ...
    def evaluate(self, gap_id: str, tension_before_release: float, release_satisfaction: int = 3) -> GateResult:
