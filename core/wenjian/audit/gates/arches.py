    ...
    def evaluate(self, position_pct: float, scar_revealed: bool) -> GateResult:
        if position_pct > 33 and not scar_revealed:
            return self.fail_result(
                message=f"故事已过 {position_pct:.0f}%，角色核心伤疤未揭示",
                details={"position": position_pct, "scar_revealed": False},
            )
...
class ARC02_DesireConflict(BaseGate):
    ...
    def evaluate(
        self, conscious_desire: str, unconscious_need: str, conflict_score: int = 0
    ) -> GateResult:
        if conflict_score < 3:
            return self.fail_result(
                message=f"欲望('{conscious_desire}')与需求('{unconscious_need}')冲突度 {conflict_score}/5，两者没有真正的矛盾",
                details={
                    "desire": conscious_desire,
                    "need": unconscious_need,
...
class ARC03_WeaknessCost(BaseGate):
    ...
    def evaluate(self, weakness: str, cost_scenes: int, total_scenes: int) -> GateResult:
        if cost_scenes < 1:
            return self.fail_result(
                message=f"弱点'{weakness}'没有造成任何实际代价——读者不会觉得它是个弱点",
                details={"weakness": weakness, "cost_scenes": 0},
            )
        if total_scenes > 20 and cost_scenes < 2:
            return self.fail_result(
                message=f"整本书只展现了 {cost_scenes} 次弱点的代价，建议至少 2-3 次递进",
                details={"weakness": weakness, "cost_scenes": cost_scenes, "total": total_scenes},
            )
...
class ARC04_Transformation(BaseGate):
    ...
    def evaluate(self, has_climax_change: bool, initial_flaw: str, final_state: str) -> GateResult:
        ...
        if initial_flaw == final_state:
            return self.fail_result(
                message="角色从头到尾没有改变——这不是角色弧光，是角色静止",
...
class ARC05_LengthAwarePacing(BaseGate):
    ...
    def evaluate(
        self, projected_chapters: int, current_chapter: int, chapters_since_arc_reset: int
    ) -> GateResult:
        if projected_chapters > 500:
            max_reset = {"short": 10, "medium": 20, "long": 5}.get(
                "short"
                if projected_chapters < 100
                else "medium"
                if projected_chapters < 500
                else "long",
                10,
            ...
            if chapters_since_arc_reset > max_reset * 2:
                return self.fail_result(
                    message=f"长篇 ({projected_chapters} 章) 需要周期性的弧线重置——当前弧持续了 {chapters_since_arc_reset} 章",
                    details={
                        "projected": projected_chapters,
                        "since_reset": chapters_since_arc_reset,
                    },
                )
...
class ARC06_PayoffDelay(BaseGate):
    ...
    def evaluate(self, longest_setup_chapters: int, projected_chapters: int) -> GateResult:
        ...
        if ratio > limit:
            return self.fail_result(
                message=f"最长铺垫弧 {longest_setup_chapters} 章占总长 {ratio:.0%}，{self._length_label(projected_chapters)}不应超过 {limit:.0%}",
                details={
                    "setup": longest_setup_chapters,
                    "total": projected_chapters,
                    "ratio": round(ratio, 2),
                    "limit": limit,
                },
            )
...
class ARC07_TurnQuality(BaseGate):
    ...
    def evaluate(
        self, surprise: int, inevitability: int, impact: int, has_chain_reaction: bool
    ) -> GateResult:
        ...
        if issues:
            return self.fail_result(
                ...
                details={
                    ...
                    "impact": impact,
                    "chain": has_chain_reaction,
                },
            )
        return self.pass_result(
            details={"total": total, "quality": "high" if total >= 12 else "medium"}
        )
...
class ARC08_ChapterBreak(BaseGate):
    ...
    def evaluate(
        ...
        has_unresolved: bool,
        has_new_info: bool,
        is_vip_chapter: bool = False,
        has_pleasure_near_end: bool = False,
    ) -> GateResult:
        issues = []
        ...
        if issues:
            return self.fail_result(
                message="；".join(issues),
                details={"unresolved": has_unresolved, "new_info": has_new_info},
            )
...
class ARC09_DepthCheck(BaseGate):
    ...
    def evaluate(self, last_deep_hook_ago: int) -> GateResult:
        if last_deep_hook_ago >= 10:
            return self.fail_result(
                message=f"距上次深度钩子已 {last_deep_hook_ago} 章——建议在最近一章设一个强钩子",
                details={"since_last_hook": last_deep_hook_ago},
            )
