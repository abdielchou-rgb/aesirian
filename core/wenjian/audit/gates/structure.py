...
class STR02_SceneObjective(BaseGate):
    ...
    def evaluate(
        self, scene_id: str, has_irreplaceable_purpose: bool, purpose_category: str = ""
    ) -> GateResult:
        if not has_irreplaceable_purpose:
            return self.fail_result(
                ...
                details={
                    "scene": scene_id,
                    "suggestion": "问问：这个场景要么推进剧情、要么塑造角色、要么提供关键信息。如果都不满足，删掉它。",
                },
            )
...
class STR03_SceneStructure(BaseGate):
    ...
    def evaluate(
        self, has_goal: bool, has_obstacle: bool, has_result: bool, has_turn: bool
    ) -> GateResult:
        missing = []
        if not has_goal:
            missing.append("角色目标")
        ...
        if missing:
            return self.fail_result(
                message=f"场景结构不完整，缺少：{'、'.join(missing)}",
                details={
                    "elements": {
                        "goal": has_goal,
...
class STR04_SevenLayerRhythm(BaseGate):
    ...
    def evaluate(self, recent_chapter_types: list, current_position: int) -> GateResult:
        ...
        if climax_positions and current_position - climax_positions[-1] > 7:
            return self.fail_result(
                message="距上次'扬/爆'章已超过 7 章，读者可能进入阅读疲劳期",
...
class STR06_ChapterLengthConsistency(BaseGate):
    ...
    def evaluate(
        self, current_length: int, avg_length: float, max_deviation_pct: float = 0.5
    ) -> GateResult:
        if avg_length == 0:
            return self.pass_result()
