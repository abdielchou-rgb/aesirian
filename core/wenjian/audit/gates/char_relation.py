    ...
    def evaluate(self, character_name: str, relationship_count: int) -> GateResult:
        if relationship_count < 2:
            return self.fail_result(
                message=f"角色'{character_name}'仅有 {relationship_count} 段实质性关系（需要至少 2 段）——孤立角色难以驱动故事",
                details={"character": character_name, "relationships": relationship_count},
            )
...
class CRN02_RelationDiversity(BaseGate):
    ...
    def evaluate(
        self, character_name: str, conflict_relationships: int, support_relationships: int
    ) -> GateResult:
        if conflict_relationships == 0:
            return self.fail_result(
                message=f"角色'{character_name}'完全没有冲突关系——没有冲突的关系不会推动故事",
                details={"conflict": 0, "support": support_relationships},
            )
        if support_relationships == 0:
            return self.fail_result(
                message=f"角色'{character_name}'完全没有支持关系——角色在故事中缺乏情感锚点",
                details={"conflict": conflict_relationships, "support": 0},
            )
        return self.pass_result(
            details={"conflict": conflict_relationships, "support": support_relationships}
        )
...
class CRN03_RelationshipEvolution(BaseGate):
    ...
    def evaluate(self, characters_with_arcs: int, total_characters: int) -> GateResult:
        if total_characters >= 3 and characters_with_arcs < total_characters:
            ...
            return self.fail_result(
                message=f"{static}/{total_characters} 个角色没有关系弧——每段关系应该在故事中发生变化",
                details={
                    "with_arc": characters_with_arcs,
                    "total": total_characters,
                    "static": static,
                },
            )
        return self.pass_result(
            details={"with_arc": characters_with_arcs, "total": total_characters}
        )
...
class CRN04_CharacterTriangle(BaseGate):
    ...
    def evaluate(self, has_protagonist: bool, has_antagonist: bool, has_ally: bool) -> GateResult:
        ...
        if not has_antagonist:
            missing.append("对手(对抗力量)")
        if not has_ally:
            missing.append("盟友(情感锚点)")
        if missing:
            return self.fail_result(
                message=f"铁三角不完整，缺少：{'、'.join(missing)}",
                details={
                    "protagonist": has_protagonist,
                    "antagonist": has_antagonist,
                    "ally": has_ally,
                },
            )
        return self.pass_result()
...
class CRN05_OpponentDepth(BaseGate):
    ...
    def evaluate(
        self, has_motivation: bool, motivation_clarity: int, shares_goal_with_protagonist: bool
    ) -> GateResult:
        if not has_motivation:
            return self.fail_result(
                message="对手没有自己的动机——好的对手相信自己才是对的",
                details={"has_motivation": False},
            )
        if motivation_clarity < 3:
            return self.fail_result(
                message=f"对手动机明确度 {motivation_clarity}/5——读者需要理解他为什么这么做",
                details={"clarity": motivation_clarity},
            )
        if shares_goal_with_protagonist:
            return self.pass_result(
                details={
                    "motivation": motivation_clarity,
                    "note": "对手与主角追求同一目标——这是高质量的对手设计",
                }
            )
