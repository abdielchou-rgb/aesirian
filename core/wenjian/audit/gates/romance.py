    ...
    def evaluate(self, tension_count: int, word_count: int) -> GateResult:
        ...
        if density < 1.0 and word_count > 500:
            return self.fail_result(
                message=f"情感张力密度 {density:.1f}/千字——每千字至少1次暧昧/误会/推拉",
                details={"tension_count": tension_count, "density": round(density, 1)},
            )
        ...
        if issues:
            return self.fail_result(
                ...
                details={
                    "sweet": sweet_count,
                    "bitter": bitter_count,
                    "consecutive_bitter": consecutive_bitter,
                },
            )
...
class ROE03_CPChemistry(BaseGate):
    ...
    def evaluate(
        self, meaningful_interactions: int, total_interactions: int, unique_scenarios: int
    ) -> GateResult:
        if total_interactions >= 3 and meaningful_interactions / total_interactions < 0.3:
            return self.fail_result(
                message=f"有效互动占比 {meaningful_interactions}/{total_interactions}——CP互动的每句话应该有潜台词",
                details={"meaningful": meaningful_interactions, "total": total_interactions},
            )
        if total_interactions >= 3 and unique_scenarios < 2:
            return self.fail_result(
                message=f"CP互动场景单一（{unique_scenarios}种）——建议在不同场景下展现关系",
                details={"scenarios": unique_scenarios},
            )
        return self.pass_result(
            details={
                "meaningful_ratio": round(meaningful_interactions / max(total_interactions, 1), 2)
            }
        )
...
class ROE04_MisunderstandingCycle(BaseGate):
    ...
    def evaluate(self, same_misunderstanding_count: int, resolved: bool) -> GateResult:
        if same_misunderstanding_count >= 3 and not resolved:
            return self.fail_result(
                message=f"同一个误会循环使用了 {same_misunderstanding_count} 次仍未解决——读者已经看懂了，角色还在误会",
                details={"count": same_misunderstanding_count, "resolved": False},
            )
        if same_misunderstanding_count >= 5:
            return self.fail_result(
                message=f"同一个误会使用了 {same_misunderstanding_count} 次——严重消耗读者耐心",
                details={"count": same_misunderstanding_count},
            )
...
class ROE05_DetailSugar(BaseGate):
    ...
    def evaluate(self, detail_sugar_count: int, emotional_beat_count: int) -> GateResult:
        if emotional_beat_count > 0 and detail_sugar_count < emotional_beat_count:
            return self.fail_result(
                message=f"情感推进 {emotional_beat_count} 次，细节糖仅 {detail_sugar_count}——女频的情感靠细节展现，不是靠情节推动",
                details={
                    "detail_sugar": detail_sugar_count,
                    "emotional_beats": emotional_beat_count,
                },
            )
...
class ROE06_LoveRivalDensity(BaseGate):
    ...
    def evaluate(
        self, love_interest_count: int, rivalry_scenes: int, chapters_covered: int
    ) -> GateResult:
        if love_interest_count >= 3 and rivalry_scenes < 2 and chapters_covered > 10:
            return self.fail_result(
                message=f"有 {love_interest_count} 条感情线但 {chapters_covered} 章内仅有 {rivalry_scenes} 场修罗场——存在多角关系但没有碰撞",
                details={"interests": love_interest_count, "rivalry": rivalry_scenes},
            )
        return self.pass_result(
            details={
                "interests": love_interest_count,
                "rivalry_per_10ch": round(rivalry_scenes / max(chapters_covered / 10, 1), 1),
            }
        )