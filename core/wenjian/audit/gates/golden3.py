    ...
    def evaluate(self, conflict_position: int) -> GateResult:
        if conflict_position > 500:
            return self.fail_result(
                ...
                details={"position": conflict_position, "deadline": 500, "overdue_by": conflict_position - 500},
            )
...
class G3_02_ProtagonistIntro(BaseGate):
    ...
    def evaluate(self, appears_in_chapter_1: bool) -> GateResult:
        if not appears_in_chapter_1:
            return self.fail_result(message="主角未在第一章出场——读者不知道谁是主角", details={"appears": False})
...
class G3_03_WhyQuestion(BaseGate):
    ...
    def evaluate(self, hook_count: int) -> GateResult:
        if hook_count < 1:
            return self.fail_result(message="第一章没有留下让读者追问的悬念", details={"hook_count": 0})
        if hook_count == 1:
            return self.pass_result(details={"hook_count": 1, "note": "刚好达标，建议增加到 2 个以上"})
...
class G3_04_Chapter2Goal(BaseGate):
    ...
    def evaluate(self, has_short_term_goal: bool, goal_clarity: int = 3) -> GateResult:
        if not has_short_term_goal:
            return self.fail_result(message="第二章结束时主角没有明确的短期目标——读者不知道主角要做什么", details={"has_goal": False})
        if goal_clarity < 3:
            return self.fail_result(message=f"目标明确度 {goal_clarity}/5，建议让目标更具体", details={"clarity": goal_clarity})
...
class G3_05_Chapter3Pleasure(BaseGate):
    ...
    def evaluate(self, has_first_pleasure: bool, pleasure_type: str = "") -> GateResult:
        if not has_first_pleasure:
            return self.fail_result(message="第三章没有爽点——三章了读者还没尝到甜头，可能会弃书", details={"has_pleasure": False})
