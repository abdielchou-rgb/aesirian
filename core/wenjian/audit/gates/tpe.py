    ...
    def evaluate(self, direct_emotions: int, touchpoints: int) -> GateResult:
        if direct_emotions > touchpoints:
            return self.fail_result(
                ...
                details={"direct_emotions": direct_emotions, "touchpoints": touchpoints, "ratio": f"{direct_emotions}:{touchpoints}"},
            )
        return self.pass_result(details={"direct_emotions": direct_emotions, "touchpoints": touchpoints})
...
class TPE02_TurnNoTouchpoint(BaseGate):
    ...
    def evaluate(self, scene_id: str, touchpoint_count: int, is_critical: bool = True, min_touchpoints: int = 2) -> GateResult:
        if is_critical and touchpoint_count < min_touchpoints:
            return self.fail_result(
                message=f"关键场景'{scene_id}'仅有{touchpoint_count}个触点（需要{min_touchpoints}个）",
                details={"scene_id": scene_id, "touchpoint_count": touchpoint_count, "min_required": min_touchpoints},
            )
        return self.pass_result(details={"scene_id": scene_id, "touchpoint_count": touchpoint_count})
    ...
    def evaluate(self, scene_id: str, touchpoint_emotion: str, context_emotion: str) -> GateResult:
        if touchpoint_emotion != context_emotion:
            return self.fail_result(
                ...
                details={"scene_id": scene_id, "touchpoint_emotion": touchpoint_emotion, "context_emotion": context_emotion},
            )
        return self.pass_result(details={"scene_id": scene_id, "touchpoint_emotion": touchpoint_emotion, "context_emotion": context_emotion})