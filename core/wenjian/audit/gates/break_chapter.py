...
class BRK01_ChapterEndHook(BaseGate):
    ...
    def evaluate(self, has_cliffhanger: bool, has_unresolved: bool, end_type: str = "") -> GateResult:
        ...
        if has_cliffhanger and has_unresolved:
            return self.pass_result(details={"end_type": "cliffhanger+unresolved", "strength": "strong"})
...
class BRK03_ChapterLength(BaseGate):
    ...
    def evaluate(self, char_count: int, genre: str = "xianxia_modern") -> GateResult:
        ...
        if issues:
            return self.fail_result(message="；".join(issues), details={"char_count": char_count, "min": 2000, "max": 3000})
...
class BRK04_FiveWHook(BaseGate):
    ...
    def evaluate(self, hook_type: str, hook_strength: int = 3) -> GateResult:
        ...
        type_names = {"consequence": "后果钩", "discovery": "发现钩", "decision": "决定钩", "crisis": "危机钩", "cliffhanger": "悬念钩"}

        if hook_type not in valid_types:
            return self.fail_result(
                message=f"章末钩子类型不明——使用五类钩子之一：后果/发现/决定/危机/悬念",
        ...
        return self.pass_result(details={"hook_type": hook_type, "type_name": type_names.get(hook_type), "strength": hook_strength})