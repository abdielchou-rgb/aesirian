...
class Planner:
    ...
    def list_templates(self) -> list[dict]:
        return [
            {"id": k, "name": v["name"], "stages": len(v["stages"]), "source": v["source"]}
            ...
            "acts": [
                {
                    "number": i + 1,
                    "beat": beats["stages"][i],
                    "description": f"第{i + 1}阶段：{beats['stages'][i]}",
    ...
    def _calc_chapter_range(
        self, stage_index: int, total_stages: int, total_chapters: int
    ) -> tuple:
