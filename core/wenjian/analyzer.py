...
FILLER_PATTERN = re.compile(
    r"(基本上|实际上|非常|真的|有点|某种|似乎|好像|几乎|开始|然后|于是|接着)"
)
...
class LocalAnalyzer:
    def analyze_chapter(self, text, chapter_index=0, title=""):
        ...
        return {
            ...
            "gap_density": gaps["density"],
            "gap_intensities": gaps["intensities"],
            "words_since_last_gap": gaps["words_since_last"],
            "first_hook_position": hook_pos,
            "has_hook": hook_pos < 99999,
    ...
    def analyze_full_manuscript(self, chapters):
        all_r = [
            self.analyze_chapter(ch["text"], i, ch.get("title", ""))
        ...
        ctx = {
            ...
            "total_chars": sum(r["char_count"] for r in all_r),
            "chapter_results": all_r,
            "gap_densities": [r["gap_density"] for r in all_r],
            ...
            "avg_hook_density": round(
                sum(r["hook_density"] for r in all_r) / max(len(all_r), 1), 2
            ),
            "avg_adverb_density": round(
                sum(r["adverb_density"] for r in all_r) / max(len(all_r), 1), 2
            ),
            "avg_passive_density": round(
                sum(r["passive_density"] for r in all_r) / max(len(all_r), 1), 2
            ),
        }
        for r in all_r:
    ...
    def _count_emotions(self, text):
        return sum(text.count(w) for w in EMOTION_WORDS)

    def _count_touchpoints(self, text):
        return sum(sum(text.count(w) for w in words) for words in TOUCHPOINT_KEYWORDS.values())
    ...
    def _count_gaps(self, text):
        ...
        cg = 0
        for p in paras:
            if any(m in p for m in GAP_MARKERS):
                cg = 0
            else:
                cg += len(p)
        return {
            "total": count,
            "density": round(count / max(len(text) / 1000, 1), 2),
            "intensities": [1] * count
            if count < 10
            else [2] * (count // 2) + [1] * (count - count // 2),
            "words_since_last": cg,
        }
    ...
    def _extract_names(self, text):
        return list(set(re.findall(r"[「『]([^」』]{2,3})[」』]", text)))

    def _count_dialogues(self, text):
        return len(DIALOGUE_LINE.findall(text))
    ...
    def _chapter_end_force(self, text):
        ...
        signals = []
        if any(w in tail for w in ["来不及", "追上", "包围", "逼近", "危险", "生死"]):
            signals.append("crisis")
        if any(w in tail for w in ["发现", "揭穿", "暴露", "竟然是", "原来"]):
            signals.append("discovery")
        if any(w in tail for w in ["决定", "我要", "必须", "一定要"]):
            signals.append("decision")
        if any(w in tail for w in ["不知道", "谁能", "到底", "难道是", "你知", "什么"]):
            signals.append("suspense")
        if any(w in tail for w in ["有件事", "有个秘密", "其实", "真相"]):
            signals.append("before_turn")
        return {
            "length": min(len(text), 300),
            "force": "strong" if len(signals) >= 2 else "moderate" if len(signals) == 1 else "weak",
            "signals": signals,
            "has_cliffhanger": len(signals) >= 1,
        }

    def _detect_scene_beats(self, text, paragraphs):
        beats = []
        for i, para in enumerate(paragraphs):
            pp = round(i / max(len(paragraphs), 1) * 100, 0)
            bt = "narration"
            if any(m in para for m in GAP_MARKERS):
                bt = "disaster"
            elif any(p in para for p in ["决定", "选择", "于是", "既然如此"]):
                bt = "decision"
            elif any(w in para for w in ["感到", "觉得", "难过", "痛苦", "恐惧"]):
                bt = "reaction"
            elif any(p in para for p in ["怎么办", "如何", "困境"]):
                bt = "dilemma"
            elif DIALOGUE_LINE.search(para):
                bt = "dialogue"
        ...
        return {
            "beat_types": list(set(types)),
            "has_scene_structure": has_conf,
    ...
    def _micro_tension_analysis(self, text, paragraphs):
        if len(paragraphs) < 3:
            return {"tension_variance": 0, "high_tension_segments": 0, "low_tension_segments": 0}
        scores = []
        ...
        high = sum(1 for s in scores if s > avg * 1.5)
        low = sum(1 for s in scores if s < avg * 0.5)
        return {
            "tension_variance": round(var, 2),
            "high_tension_segments": high,
            "low_tension_segments": low,
            "is_flat": high == 0 and low == 0,
        }
            ...
            segs.append(
                {
                    ...
                    "hook_density": round(
                        sum(seg.count(h) for h in HOOK_MARKERS) / max(len(seg) / 1000, 1), 2
                    ),
                }
            )
        return segs
