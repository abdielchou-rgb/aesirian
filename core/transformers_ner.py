...
class EntityExtractor:
    ...
    def __init__(self, use_transformers: bool = True):
        ...
        if self.use_transformers:
            ...
            _warn_once(
                "entity_extractor_regex_fallback",
                "EntityExtractor: Falling back to regex-only mode",
            )

    @staticmethod
    def _merge_ner_with_regex(
        _text: str, ner_entities: list[dict], regex_result: ExtractionResult
    ) -> ExtractionResult:
        ...
        for ent in ner_entities:
            ...
            if entity_type == "character" and ent["word"] not in regex_result.characters:
                ...
                regex_result.events.append(
                    ExtractedEntity(name=ent["word"], entity_type="event", confidence=confidence)
                )
            elif entity_type in ("item", "organization", "product") and ent["word"] not in regex_result.items:
    ...
    def _regex_extract(text: str) -> ExtractionResult:
        ...
        for m in re.finditer(
            r"([一-鿿]{2,4})(?:说|道|问|答|喊|叫|哭|笑|叹|走|跑|看|望|推|拉|坐|站)", text
        ):
            candidate = m.group(1)
        ...
        # 常见姓氏 + 名
        for m in re.finditer(
            r"(?:李|王|张|刘|陈|赵|钱|孙|周|吴|郑|冯|林|沈|叶|秦|宋|顾|白|韩|苏|黄|高|马|郭|唐|曹|许|邓|萧|段|顾|沈|林)([一-鿿]{1,2})(?:\s|，|。|？|！|的|了|在|是|有|说|道|看|走|笑|哭|坐|站|拿)",
            text,
        ):
        ...
        locations = set()
        for m in re.finditer(
            r"(?:在|到|从|去|回|往|向|路过|经过)([一-鿿]{2,6})(?:里|上|中|旁|边|前|后|内|外|附近|的)",
            text,
        ):
            locations.add(m.group(1))
        for m in re.finditer(
            r"(?:门口|窗外|街上|楼上|屋里|门外|路口|村口|山下|河边|海边|城门口|房间|大厅|花园|街道|广场|医院|学校|公司)",
            text,
        ):
        ...
        events = set()
        for m in re.finditer(
            r"([一-鿿]{2,10}(?:案|事|事件|事故|战争|灾难|聚会|仪式|考试|战斗|冲突|谈判))", text
        ):
        ...
        items = set()
        for m in re.finditer(
            r"([一-鿿]{2,4})(?:刀|剑|枪|书|信|包|纸|盒|钥匙|手机|钱包|戒指|项链|玉佩)", text
        ):
        ...
        for m in re.finditer(r"([一-鿿]{2,4})今(?:年|日|岁)(?:已经|才|刚|就)?(\d+)(?:岁|了)", text):
            result.facts.append(
                ExtractedFact(
                    subject=m.group(1), predicate="年龄", obj=int(m.group(2)), confidence=0.9
                )
            )
        for m in re.finditer(
            r"([一-鿿]{2,4})(?:有|是|已|已经)(\d+)(?:岁|年|天|月|斤|米|厘米|个|次)", text
        ):
            if not any(skip in m.group(0) for skip in ["可以", "没有", "因为", "所以", "一些"]):
                result.facts.append(
                    ExtractedFact(
                        subject=m.group(1), predicate="数量", obj=int(m.group(2)), confidence=0.8
                    )
                )
        ...
        for char_name in char_mentions:
            if char_name in SKIP_WORDS or len(char_name) > 3:
                continue
            for m in re.finditer(
                re.escape(char_name) + r"([一-鿿]{0,4}?(?:" + "|".join(ACTION_VERBS) + r"))", text
            ):
        ...
        for char_name in char_mentions:
            ...
            for m in re.finditer(
                re.escape(char_name)
                + r"(?:从|离开)([一-鿿]{2,6})(?:里|上|中)?(?:到|回|去|来|返回|前往|走向)([一-鿿]{2,6})",
                text,
            ):
                result.movements.append(
                    Movement(character=char_name, from_location=m.group(1), to_location=m.group(2))
                )

        # ── POV切换检测 ──
        pov_markers = set()
        for _m in re.finditer(r"(?:我|我们)(?:觉得|认为|想|知道|看见|听到|感到)", text):
            pov_markers.add("第一人称")
        for _m in re.finditer(r"(?:他|她|它|他们)(?:觉得|认为|想|知道|看见|听到|感到)", text):
        ...
        for prop in IDENTITY_PROPERTIES:
            for m in re.finditer(
                r"([一-鿿]{2,4})"
                + prop
                + r"(?:变成|成为|变了|不再是|突然变得)([一-鿿，。！？\s]{2,20})",
                text,
            ):
                result.identity_changes.append(
                    {
                        ...
                        "has_explanation": bool(
                            re.search(r"因为|由于|原来|经过|经历了|发现", m.group(2))
                        ),
                    }
                )
    ...
    def to_gate_context(text: str) -> dict:
        ...
        return {
            ...
            "char_actions": [
                {"character_id": a.character, "description": a.action} for a in result.actions
            ],
            "identity_changes": result.identity_changes,
            "events": [{"event": e.name, "reference_chapter": 0} for e in result.events],
            "movements": [
                {
