...
from core.planning.outline import (
    OutlineNode, StoryOutline, TEMPLATE_STRUCTURES,
)
...
class OutlineGenerator:
    ...
    def generate(self, premise: str, template: str = "three_act",
                 target_chapters: int = 12) -> StoryOutline:
        ...
        root = OutlineNode(id="root", level="act", title=premise[:30] or "未命名",
                           description=premise, children=acts)
                ...
                if m:
                    try:
                        ...
                        for i, s in enumerate(structure):
                            ...
                            acts.append(OutlineNode(
                                id=f"act_{i+1}", level="act", title=title,
    ...
    def _generate_chapters(self, act: OutlineNode, premise: str,
                           n: int, counter: list) -> list[OutlineNode]:
                    ...
                    try:
                        for it in json.loads(m.group(0))[:n]:
                            if not it.get("title"):
                                continue
                            counter[0] += 1
                            chapters.append(OutlineNode(
                                id=f"ch_{counter[0]:03d}", level="chapter",
    ...
    def _annotate_story_values(self, node: OutlineNode):
        ...
        for a, b in zip(node.children, node.children[1:]):
            if a.story_value != "neutral" and b.story_value != "neutral" and a.story_value != b.story_value:
