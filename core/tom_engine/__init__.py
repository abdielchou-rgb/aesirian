...
class TheoryOfMindEngine:
    ...
    - 信念更新（目击/二手/欺骗/误解）
    - 递归嵌套推理（A认为B以为C知道D）
    - 戏剧张力点检测（优化版：使用倒排索引 O(P × C²) 而非 O(C² × P)）
    - 角色行动倾向推理
    """
    ...
    def __init__(self):
        ...
        # ═══════════════════════════════════════
        # 倒排索引：命题 → {char_id: Belief}
        # 用于 O(1) 查找"哪些角色持有某命题的信念"
        # ═══════════════════════════════════════
        self._belief_index: dict[str, dict[str, Belief]] = {}  # proposition -> {char_id: Belief}
    ...
    def _rebuild_index(self):
        ...
        for cid, char in self.characters.items():
            for prop, belief in char.world_beliefs.items():
                if prop not in self._belief_index:
                    self._belief_index[prop] = {}
                self._belief_index[prop][cid] = belief
    ...
    def detect_tension(self) -> list[TensionPoint]:
        ...
        优化：使用倒排索引 _belief_index
        - 旧算法：O(C² × P) 其中 C=角色数, P=平均命题数
        - 新算法：O(P × C²) 但只遍历有冲突的命题，实际复杂度大幅降低
        """
        self._ensure_index()
        self.tension_points = []

        # 检测信念冲突：遍历每个命题，找出持有不同值的角色对
        for prop, holders in self._belief_index.items():
            if len(holders) < 2:
                continue  # 只有一个角色持有该信念，无冲突

            # 按信念值分组
            value_groups: dict = {}
            for cid, belief in holders.items():
                if belief.is_erroneous:
                    continue  # 错误信念不产生冲突（由欺骗/误解产生）
                val = belief.value
                if val not in value_groups:
                    value_groups[val] = []
                value_groups[val].append((cid, belief))
