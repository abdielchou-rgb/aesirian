...
class AuditDimension:
    ...
    # 如何检查——由门禁系统实现
    check_fn: Optional[str] = None  # 对应的方法名
...
DIMENSION_REGISTRY: dict[str, AuditDimension] = {
    ...
    # ====== 悬疑推理专属 =======
    "D10": AuditDimension("D10", "时序精密性", "时间线必须精确可追溯，不允许模糊时间跳跃",
                          active_by_default=False, weight=2.0,
                          prompt_note="悬疑推理要求时间线精确到天甚至小时。检查时间表述是否模糊"),
    "D11": AuditDimension("D11", "红鲱鱼密度", "假线索与真线索的比例合理",
                          active_by_default=False, weight=1.5,
                          prompt_note="悬疑推理需要红鲱鱼（假线索）。检查是否有足够的分歧方向"),
    "D12": AuditDimension("D12", "线索回路完整性", "所有线索必须闭环或显式保留",
                          active_by_default=False, weight=2.0,
                          prompt_note="悬疑推理不允许'失落的线索'——每一条线索要么回收，要么显式标注为待续"),

    # ====== 仙侠玄幻专属 =======
    "D20": AuditDimension("D20", "战力体系一致性", "战力等级体系不能乱",
                          active_by_default=False, weight=2.0,
                          prompt_note="修仙境界/等级体系必须严格遵守。越级挑战必须有合理解释"),
    "D21": AuditDimension("D21", "升级节奏", "主角突破/升级间隔合理",
                          active_by_default=False, weight=1.5,
                          prompt_note="每10-15章一个升级节奏。太快=水分，太慢=拖沓"),
    "D22": AuditDimension("D22", "世界观规则", "世界观设定的一致性和约束",
                          active_by_default=False, weight=2.0,
                          prompt_note="仙侠世界的物理/魔法规则不可随意打破"),

    # ====== 都市逆袭专属 =======
    "D30": AuditDimension("D30", "爽点密度", "打脸/逆袭/碾压频率达标",
                          active_by_default=False, weight=1.5,
                          prompt_note="都市逆袭每3章至少1个爽点。检查最近章节的爽点密度"),
    "D31": AuditDimension("D31", "逆袭节奏", "被压制→机会→逆袭的循环节奏",
                          active_by_default=False, weight=1.5,
                          prompt_note="逆袭需要'低点→转折→高潮'的完整弧线，不能一路顺风"),
    "D32": AuditDimension("D32", "资源增长曲线", "经济/人脉/地位增长合理",
                          active_by_default=False, weight=1.0,
                          prompt_note="逆袭流的资源增长需要曲线而非直线"),

    # ====== 言情专属 =======
    "D40": AuditDimension("D40", "情感弧线完整性", "相遇→吸引→障碍→牺牲→重聚",
                          active_by_default=False, weight=2.0,
                          prompt_note="言情需要完整的情感弧线。检查是否有跳过关键阶段"),
    "D41": AuditDimension("D41", "甜虐比", "甜蜜与虐心的比例合理",
                          active_by_default=False, weight=1.5,
                          prompt_note="甜虐比约3:1。过虐劝退，过甜无张力"),
    "D42": AuditDimension("D42", "CP互动频率", "主要CP的互动密度",
                          active_by_default=False, weight=1.0,
...
class AuditDimensionManager:
    ...
    def __init__(self, genre: Genre = Genre.GENERAL):
        self.genre = genre
        self._active_cache: Optional[list[AuditDimension]] = None
    ...
    def get_active_dimensions(self) -> list[AuditDimension]:
        ...
        self._active_cache = [
            DIMENSION_REGISTRY[did] for did in dim_ids
            if did in DIMENSION_REGISTRY
        ]
        return self._active_cache

    def get_prompt_block(self) -> str:
        """生成注入LLM审计prompt的维度说明块"""
        lines = ["【可配置审计维度 — 当前genre: {}】".format(self.genre.value)]
