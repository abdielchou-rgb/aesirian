...
class ReaderModelSimulator:
    ...
    def predict_confusion(self, operations: list[dict]) -> Optional[str]:
        """预测一组叙事操作是否会导致读者困惑"""
        total_cost = sum(self.estimate_update_cost(op) for op in operations)
        if total_cost > 12:
            return (
                f"连续叙事操作的累计认知成本为{total_cost}（阈值12），"
                f"可能让读者感到困惑"
            )
        return None
    ...
    def get_trend(self) -> str:
        ...
        recent = self.transportation_history[-3:] if len(self.transportation_history) >= 3 else self.transportation_history
...
class EventCooldownMatrix:
    ...
    def get_hot_patterns(self, threshold: float = 0.5) -> list[tuple[str, float]]:
        ...
        return [
            (k, v) for k, v in sorted(
                self.matrix.items(), key=lambda x: -x[1]
            )
            if v >= threshold
        ]
    ...
    def get_cold_patterns(self, threshold: float = 1.0) -> list[str]:
        """获取空闲可用的模式"""
        all_patterns = set(
            self.PLEASURE_TYPES + self.CONFLICT_TYPES + self.EMOTIONAL_ARC_TYPES
        )
        used = set(self.matrix.keys())
        cold = [
            p for p in all_patterns - used
            if self.matrix.get(p, 0.0) < threshold
        ]
        return cold
    ...
    def check_saturation(self, window: int = 5, threshold: float = 0.6) -> Optional[str]:
        """检查是否存在模式饱和（重复率过高）"""
        recent = self.usage_history[-window:] if self.usage_history else []
