...
class TestE2E:
    ...
    def test_full_product_flow(self):
        ...
        r = client.post("/import-from-pwa", json={
            ...
            "characters": [{"name": "沈夜", "role": "心理医生"}, {"name": "林晚", "role": "患者"}],
            "tone": "悬疑暗流", "conflict": "认知冲突",
        })
        ...
        # 3. AI 续写（LLM 依赖，允许降级）
        cont = client.post("/project/" + pid + "/ai-continue", json={
            "project_id": pid,
        ...
        # 4. 提交章节 → 审计（含质量检测）
        sub = client.post("/project/" + pid + "/submit-chapter", json={
            "project_id": pid,
        ...
        aud = client.post("/project/" + pid + "/audit", json={
            "project_id": pid,
        ...
        # 6. 推演（克隆不污染）
        sim = client.post("/project/" + pid + "/simulate", json={
            "hypothesis": "如果沈夜发现林晚也在撒谎", "belief_changes": [],
        })
        assert sim.status_code == 200 and sim.json()["branches"]

        # 7. 发散
        div = client.post("/diverge", json={
            "fragments": ["深海走廊的尽头有光", "档案里画着同一扇门", "禁止提及四个字"],
        })
        assert div.status_code == 200 and len(div.json()["worldlines"]) == 5

        # 8. 大纲
        ol = client.post("/api/generate-outline", json={
            "premise": "深海空间站的心理医生发现患者共享同一个梦",
        ...
        pub = client.post("/style-profiles", json={
            "project_id": pid, "name": "深海梦境风", "genre_tags": ["悬疑"],
        })
