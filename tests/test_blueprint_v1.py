...
def _mkproject(premise="蓝图测试项目") -> str:
    r = client.post("/import-from-pwa", json={
        "premise": premise, "unit_text": premise,
        "characters": [{"name": "陈默", "role": "主角"}, {"name": "曹渊", "role": "来客"}],
        "tone": "悬疑暗流", "conflict": "生存冲突",
    })
...
class TestBlueprintV1:
    ...
    def test_ai_tell_detector_rules(self):
        ...
        d = AITellDetector()
        # 段落均匀
        uniform = "\n".join("他走了三步然后停下。然而他似乎想到了什么。所以她大概明白了。" for _ in range(3))
        # 公式化过渡
        ...
        # 干净文本零误报
        clean = ("夜色像墨一样浓。他推开那扇沉重的木门，门轴发出刺耳的呻吟，仿佛多年未曾有人来过。\n"
    ...
    def test_generate_outline(self):
        ...
        r = client.post("/api/generate-outline", json={
            "premise": "火星殖民地侦探调查密室谋杀案",
            "template": "three_act", "target_chapters": 6,
        })
        assert r.status_code == 200
        d = r.json()
        acts = d["acts"]
        ...
        log_ok(f"outline: {len(acts)} acts, {len(all_ch)} chapters, "
    ...
    def test_mice_lifo_validation(self):
        ...
        root = OutlineNode(id="root", level="act", title="t", children=[
            # 外层 idea 线程开启不闭合
            OutlineNode(id="a1", level="chapter", title="外层", mice_type="idea",
                        mice_open_chapter=1),
            # 内层 event 先闭合（合规 LIFO）
            OutlineNode(id="a2", level="chapter", title="内层", mice_type="event",
                        mice_open_chapter=2),
            OutlineNode(id="a3", level="chapter", title="内层闭", mice_type="event",
    ...
    def test_generate_with_quality(self):
        pid = _mkproject_with_chapter()
        r = client.post("/api/generate-with-quality", json={
            "project_id": pid, "instruction": "陈默发现水晶里的记忆指向一个更大的阴谋",
            "word_target": 400,
        })
        ...
        assert d["rounds"] >= 1
        log_ok(f"quality generation: score={d['score']} rounds={d['rounds']} "
