...
def _mkproject() -> str:
    r = client.post("/import-from-pwa", json={
        "premise": "产品化测试项目", "unit_text": "产品化测试项目",
...
class TestProductV1:
    ...
    def test_reality_effect(self):
        ...
        # 具体文本零误报
        concrete = ("他推开沉重的木门，门轴发出刺耳的呻吟。空气里弥漫着灰尘与旧书页的味道。"
                    "月光落在她半边脸上，她攥紧了袖口。")
    ...
    def test_control_illusion(self):
        ...
        pov_jump = ("我推开门，看见他坐在窗边。他说：你来了。我走过去。他的目光一直落在我的手上。"
                    "她觉得这很不寻常，但他没有说破。")
    ...
    def test_quality_aggregator_endpoint(self):
        pid = _mkproject()
        r = client.post("/api/quality", json={
            "project_id": pid,
    ...
    def test_audit_endpoint(self):
        pid = _mkproject()
        r = client.post("/project/" + pid + "/audit", json={
            "project_id": pid,
    ...
    def test_ai_continue(self):
        ...
        r = client.post("/project/" + pid + "/ai-continue", json={
            "project_id": pid,
        ...
        assert "score" in d
        log_ok(f"ai-continue: {len(d['text'])} chars, score={d['score']}, ctx={d['context_tokens']}")
