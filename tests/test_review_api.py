...
def test_sync_draft_completed(client):
    ...
    r = client.post(
        ...
        json={
            "source": "draft",
            "content": {
                "content": "陈默把水晶接入读数仪。突然灯灭了。但门被反锁，他绝望地握紧了刀。"
            },
            "mode": "realtime",
...
def test_lock_conflict_and_resolve_accept(client):
    ...
    # 锁定 framework
    assert (
        client.post("/api/project/p2/lock", json={"view": "framework", "locked": True}).status_code
        == 200
    )
    ...
    assert any(
        c["conflict_type"] == "lock" and c["target_view"] == "framework" for c in data["conflicts"]
    )
    sync_id = data["sync_id"]

    # 拒绝：framework 不变
    r2 = client.post(
        f"/api/project/p2/sync/{sync_id}/resolve", json={"view": "", "action": "reject"}
    )
...
def test_lock_conflict_resolve_accept_applies(client):
    ...
    r2 = client.post(
        f"/api/project/p3/sync/{sync_id}/resolve", json={"view": "framework", "action": "accept"}
    )
    assert r2.status_code == 200
...
def test_sync_mode_change(client):
    ...
    assert (
        client.post("/api/project/p4/sync-mode", json={"mode": "manual"}).json()["mode"] == "manual"
    )
