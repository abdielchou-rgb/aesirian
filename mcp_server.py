...
def dispatch_request(msg: dict) -> str | None:
    ...
    if method == "initialize":
        return json.dumps(
            {"jsonrpc": "2.0", "id": msg_id, "result": _handle_initialize(params)},
            ensure_ascii=False,
        )
    if method == "tools/list":
        return json.dumps(
            {"jsonrpc": "2.0", "id": msg_id, "result": _handle_tools_list(params)},
            ensure_ascii=False,
        )
    if method == "tools/call":
        ...
        if "error" in result:
            return json.dumps(
                {"jsonrpc": "2.0", "id": msg_id, "error": result["error"]}, ensure_ascii=False
            )
